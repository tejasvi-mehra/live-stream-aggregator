...
const App: React.FC = () => {
  ...
  const filteredEventCount = useMemo(
    () => filteredCategories.reduce((sum, category) => sum + category.events.length, 0),
    [filteredCategories]
  );
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col">
      ...
      <main className="flex-1 pb-20">
          ...
          <div className={`relative w-full h-[50vh] md:h-[70vh] overflow-hidden ${featuredUnavailable ? 'opacity-60' : ''}`}>
            ...
            <div className="absolute bottom-0 left-0 p-6 md:p-16 z-20 max-w-2xl">
              ...
              <button
                ...
                className={`px-8 py-3 rounded-md font-bold text-lg flex items-center gap-2 transition-colors disabled:opacity-60 disabled:cursor-not-allowed ${featuredUnavailable
        ...
        <div className={`px-6 md:px-16 ${featuredEvent && !isSearching && selectedCategory === 'all' && !loading ? 'pt-12' : 'pt-24'}`}>
            ...
            <div>
              ...
              <p className="text-slate-500 mt-2">
                {loading
                  ? 'Loading catalog...'
                  : isSearching
                    ? `${filteredEventCount} result${filteredEventCount === 1 ? '' : 's'} for "${searchQuery.trim()}"`
          ...
          {loading ? (
          ...
          ) : filteredCategories.some((category) => category.events.length > 0) ? (
            ...
            <div className="text-center py-20">
              <h3 className="text-xl font-bold mb-2">
                {isSearching ? 'No streams match your search' : 'No events match your filters'}
              </h3>
              <button
                onClick={() => {
                  setSearchQuery('');
